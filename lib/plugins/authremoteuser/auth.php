 ...
 * @author     Karl-Wilhelm Rips <dokuwiki.org@rips.de>
 */
class auth_plugin_authRemoteUser extends DokuWiki_Auth_Plugin {
    ...
    public function __construct() {
        ...
        $this->cando['external'] = true;  // trustExternal() is used
        $this->cando['logout']   = false; // logout() isn't possible because every
		                                  // page load re-authenticates the user
    ...
    /**
     * Check for REMOTE_USER set by webserver
     *
     * Checks if REMOTE_USER exists and is known; if so then the user
     * is already authenticated that is a password check isn't required.
     *
     * @author  Karl-Wilhelm Rips <dokuwiki.org@rips.de>
     * @param string $user
     * @param string $pass
    ...
    public function trustExternal($user, $pass, $sticky=false) {
        ...
        if (!empty($_SERVER['REMOTE_USER'])) {
            ...
            $userinfo = $this->getUserData($remoteUser);
            if($userinfo === false) {
                return false;
            } else {
